"""
CrewAI Integration for OpenClay.
...
Usage:
    ...
    from openclay.integrations.crewai import OpenClayCrewInterceptor
...
class OpenClayCrewInterceptor:
    ...
    def _handle_block(self, result: Dict, step_output: Any, stage: str):
        ...
        logger.warning(
            "OpenClay BLOCKED CrewAI %s: reason=%s",
            stage,
        ...
        if self.raise_on_block:
            raise ValueError(
                f"OpenClay blocked CrewAI {stage}: {result.get('reason', 'unknown')}"
            )
    ...
    def step_callback(self, step_output: Any) -> None:
        ...
        try:
            ...
            logger.warning("OpenClay CrewAI step scan failed: %s", e)
