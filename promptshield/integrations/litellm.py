...
Usage:
    ...
    litellm.callbacks = [PromptShieldLiteLLMCallback(shield=shield)]
...
_threat_var: contextvars.ContextVar[float] = contextvars.ContextVar(
    "promptshield_litellm_threat", default=0.0
)

class PromptShieldLiteLLMCallback(CustomLogger):
    """
    LiteLLM custom logger that enforces PromptShield at the proxy level.
    ...
    def _handle_block(self, result: Dict, stage: str):
        ...
        logger.warning(
            "PromptShield BLOCKED at %s: reason=%s",
            stage,
        ...
        if self.raise_on_block:
            raise ValueError(
                f"PromptShield blocked at {stage}: {result.get('reason', 'unknown')}"
            )
    ...
    def log_pre_api_call(self, model, messages, kwargs):
        ...
        try:
            ...
            logger.warning("PromptShield log_pre_api_call failed: %s", e)
    ...
    def _scan_response(self, response_obj, stage: str):
        ...
        try:
            ...
            logger.warning("PromptShield %s scan failed: %s", stage, e)

    # ── Failure Event (Log only) ──

    def log_failure_event(self, kwargs, response_obj, start_time, end_time):
        """Log LLM failures for observability (no blocking)."""
        logger.info("LiteLLM call failed — no PromptShield scan required")
