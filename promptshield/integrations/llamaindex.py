"""
LlamaIndex Integration for PromptShield.
...
class PromptShieldRetrieverFilter:
    """
    ...
    def _postprocess_nodes(
        ...
        """
        Filter retrieved nodes through PromptShield.
        ...
        for node in nodes:
            ...
            if result.get("blocked") or result.get("threat_level", 0.0) >= self.threat_threshold:
                ...
                logger.warning(
                    "PromptShield filtered RAG node: reason=%s, threat=%.3f",
...
if _HAS_LLAMAINDEX:
    PromptShieldRetrieverFilter.__bases__ = (BaseNodePostprocessor,)
