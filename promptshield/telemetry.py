...
class PromptShieldTelemetry:
    ...
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(PromptShieldTelemetry, cls).__new__(cls)
    ...
    def _initialize(self):
        ...
        if self.enabled:
            logger.debug("OpenTelemetry API found. Initializing PromptShield telemetry.")
            self.tracer = trace.get_tracer("promptshield")
            self.meter = metrics.get_meter("promptshield")

            self.blocks_counter = self.meter.create_counter(
                "promptshield.blocks.total",
                description="Total number of AI interactions blocked by PromptShield",
            )
            self.latency_histogram = self.meter.create_histogram(
                "promptshield.latency.ms",
                unit="ms",
                description="Latency of PromptShield scans in milliseconds",
            )
            self.threat_histogram = self.meter.create_histogram(
                "promptshield.threat.level",
...
telemetry = PromptShieldTelemetry()
...
def trace_shield_call(scan_type: str):
    """
    Decorator to automatically trace and collect metrics for PromptShield methods.
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args, **kwargs):
            ...
            with telemetry.tracer.start_as_current_span(f"promptshield.{scan_type}") as span:
                try:
                    ...
                    if isinstance(result, dict):
                        ...
                        # Span Attributes
                        span.set_attribute("promptshield.blocked", blocked)
                        span.set_attribute("promptshield.threat_level", threat_level)
                        if reason:
                            span.set_attribute("promptshield.reason", reason)
